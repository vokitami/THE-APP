...
export default function Login(){
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            //guardar  token en local sotrage
            localStorage.setItem('token', res.data.token);

            alert('login sexceseful');
  ...
  <header>
    ...
    <div className='h-[70%] relative flex w-full items-center justify-center'> 
        ...
        <p className='text-zinc-800 '>start your yorney</p>
        ...
        <input className=' h-[30px] outline-0 p-1 mb-1 mx-1' type="email" name="email" required value={email} onChange={e => setEmail(e.target.value)}/>
        </div>

        <div className='flex flex-col border-1 p-1 my-5 rounded-[5px] w-full border-zinc-600'>
            <label className='text-zinc-600 p-1'>Password</label>
            <input className='h-[30px] outline-0 p-1 mb-1 mx-1' type="password" name="password" required value={password} onChange={e => setPassword(e.target.value)}/>
        </div>
    ...
    <p>
        Don't Have an account? <a href='/register' className="text-blue-600 cursor-pointer hover:underline">Sign up</a>
    </p>
  ...
  {/* right side */}
  <div className="w-1/2 bg-gradient-to-t from-blue-950 to-cyan-500 flex items-center justify-center">
  </div>
</div>
        </>  
    )
